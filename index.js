...
app.post('/api/login', async (req, res) => {
  try {
    ...
    if (row.length == 1) {
    res.json({statusCheck: true, role: row[0].role})
    } else {
...
app.get('/api/product',async (req,res) => {
  const [dataProduct] = await db.query('SELECT * FROM quanlicuahang.products')
...
app.post('/api/product/add', async (req,res) => {
  ...
  await db.query(`INSERT INTO quanlicuahang.products (id, name, price, quantity, status) VALUES (?,?,?,?,?)`,[id,name,price,quantity,status])
  res.sendStatus(200)
})

app.post('/api/product/fix', async (req,res) => {
  const {id,name,price,quantity,status,idOld} = req.body
  await db.query(` UPDATE quanlicuahang.products SET id = ?, name = ?, price = ?, quantity = ?, status = ? WHERE id = ?`,[id,name,price,quantity,status,idOld])
  res.sendStatus(200)
})

app.post('/api/product/search', async (req,res) => {
  const {id, name} = req.body
  const [dataProduct] = await db.query(`SELECT * FROM quanlicuahang.products WHERE id = ?  OR name LIKE ? `,[id,`%${name}%`])
  res.json(dataProduct)
  res.sendStatus(200)
})
...
app.get('/api/customer',async (req,res) => {
  const [dataCustomer] = await db.query('SELECT * FROM quanlicuahang.customers')
...
app.post('/api/customer/add', async (req,res) => {
  ...
  await db.query(`INSERT INTO quanlicuahang.customers (id,fullname,birthyear,address,status) VALUES (?,?,?,?,?)`,[id,fullname,birthyear,address,status])
  res.sendStatus(200)
})

app.post('/api/customer/fix', async (req,res) => {
  const {id,fullname,birthyear,address,status,idOld} = req.body
  await db.query(` UPDATE quanlicuahang.customers SET id = ?, fullname = ?, birthyear = ?, address = ?, status = ? WHERE id = ?`,[id,fullname,birthyear,address,status,idOld])
  res.sendStatus(200)
})

app.post('/api/customer/search', async (req,res) => {
  const {id, fullname} = req.body
  const [dataCustomer] = await db.query(`SELECT * FROM quanlicuahang.customers WHERE id = ?  OR fullname LIKE ? `,[id,`%${fullname}%`])
  res.json(dataCustomer)
  res.sendStatus(200)
})
...
app.post('/api/order/fix', async (req, res) => {
  const { id, customer_id, order_date, total_price, status, idOld } = req.body;
  await db.query(
...
app.get('/api/warehouse', async (req, res) => {
  const [dataWarehouse] = await db.query('SELECT * FROM quanlicuahang.warehouses');
...
app.post('/api/warehouse/fix', async (req, res) => {
  const { id, supplier_name, import_date, total_value, status, idOld } = req.body;
  await db.query(
